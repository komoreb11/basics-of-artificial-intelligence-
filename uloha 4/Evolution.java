        ...
        for(int g=0; g < generations; g++) {
            ...
            while(newInds.size() < populationSize) {
                ...
                if(crossoverProbability < random.nextDouble()) {    
                    ...
                    Individual c1 = (Individual)offspring.a;
                    Individual c2 = (Individual)offspring.b;
                    c1.computeFitness();  //probabilistic crowding
                    c2.computeFitness();
                    int c1p1 = c1.score(parents.get(0));
                    int c1p2 = c1.score(parents.get(1));
                    int c2p1 = c2.score(parents.get(0));
                    int c2p2 = c2.score(parents.get(1));
                    if(c1p1 <=  c2p1)
                        if(c2.getFitness() < parents.get(0).getFitness())
                            offspring.a = parents.get(0).deepCopy();
                    else
                        if(c1.getFitness() < parents.get(0).getFitness())
                            offspring.a = parents.get(0).deepCopy();
                    if(c1p2 <=  c2p2)
                        if(c2.getFitness() < parents.get(1).getFitness())
                            offspring.b = parents.get(1).deepCopy();
                    else
                        if(c1.getFitness() < parents.get(1).getFitness())
                            offspring.b = parents.get(1).deepCopy();
                }
